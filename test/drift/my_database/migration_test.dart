...
void main() {
  ...
  test('migration from v1 to v2 does not corrupt data', () async {
    ...
    final oldSetsData = <v1.SetsData>[];
    final expectedNewSetsData = <v2.SetsData>[];
    ...
    await verifier.testWithDataIntegrity(
      ...
      createItems: (batch, oldDb) {
        ...
        batch.insertAll(oldDb.sets, oldSetsData);
        batch.insertAll(oldDb.users, oldUsersData);
      },
      validateItems: (newDb) async {
        expect(
            expectedNewWorkoutsData, await newDb.select(newDb.workouts).get());
        ...
        expect(expectedNewWorkoutExercisesData,
            await newDb.select(newDb.workoutExercises).get());
        expect(expectedNewSetsData, await newDb.select(newDb.sets).get());
